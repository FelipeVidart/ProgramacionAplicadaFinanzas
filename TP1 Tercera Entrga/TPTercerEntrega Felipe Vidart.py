# -*- coding: utf-8 -*-
"""
Created on Mon Apr  3 19:45:20 2023
...
class Myarray1:
    ...
    def scale_row(self, j: int, x: int, matrix_input = None):
        ...
        if type(x) != int and type(x) != float:
            print(f"ERROR: X debe ser un entero o float y se ha introducido: {type(x)}")
            new_object = None
        if j > self.r:
    ...
    def inverse(self):
        ...
        if self.r == self.c and self.det() != 0: #para poder calcular la inversa de una matriz debe ser cuadrada y el determinante debe ser distinto de 0
            ...
            end = self.return_byrow(inverse)
        else:
            print('No se puede calcular la inversa de la matriz ya sea porque no es cuadrada o porque su determinante es igual a 0')
            end = None
        return end
    
    def set_col(self, matrix, col, new_col):
        """Set col es una funcion de uso interno de la clase. Su uso es modificar los valores de una columna de una matriz. Su principal
        se lo doy en en el método "inverse gauss jordan" """
        matrix_list = matrix.to_by_column()
        col_counter = 1 
        new_matrix = []
        for i in range(0, len(matrix_list), matrix.r):
            if col_counter == col:
                new_matrix.extend(new_col) #aca creo que hay algo mal, puede ser que quede como una lista
            elif col_counter != col:
                new_matrix.extend(matrix_list[i: i+matrix.r])
            col_counter += 1
        end = matrix.return_bycolumn(new_matrix)
        return end

    def gauss_jordan(self):
        if self.r == self.c:
            main_identity = self.eye(self.r)
            matrix = self.__class__(self.r, self.c, self.elems.copy(), self.by_row)
            for counter_rc in range(1, matrix.c+1):
                row_swap_counter = counter_rc + 1
                while matrix.get_col(counter_rc)[counter_rc-1] == 0 and counter_rc != matrix.r:
                    matrix = matrix.swap_rows(counter_rc, row_swap_counter)
                    main_identity = main_identity.swap_rows(counter_rc, row_swap_counter)
                    row_swap_counter += 1
                    if counter_rc == matrix.r-1 and matrix.get_col(counter_rc)[counter_rc-1] == 0: #si se esta en la ultima run y el primero sigue siendo 0...
                        print('ERROR: No se puede calcular la inversa de una matriz cuyo determinante es igual a 0')
                        break
                matrix = matrix.scale_row(counter_rc, 1/matrix.get_elem(counter_rc, counter_rc))
                main_identity = main_identity.scale_row(counter_rc, 1/matrix.get_elem(counter_rc, counter_rc))
                new_col = []
                for index, element in enumerate(matrix.get_col(counter_rc)):
                    if index+1 == counter_rc:
                        new_col.append(element)
                    else:
                        new_col.append(-element)
                mod_identity = matrix.set_col(matrix.eye(matrix.r), counter_rc, new_col) #esto y las lineas de arriba son para cambiarle la columna a la identidad por la que necesito para que se anulen los elementos de la matriz original
                main_identity = main_identity.__class__(main_identity.r, main_identity.c, mod_identity @ main_identity, main_identity.by_row)
                matrix = matrix.__class__(matrix.r, matrix.c, mod_identity @ matrix, matrix.by_row) #las hago asi porque las tengo que convertir a instancia, podria hacerlo mas corto incluyedolo en matmul pero tengo miedo de que me rompa el resto del codigo
            end = main_identity.elems
        else:
            print('ERROR: No se puede calcular la inversa de una matriz que no es cuadrada')
            end = None
        return end
...
matriz1 = Myarray1(3,3,[1,2,7,4,1,7,7,8,1], True)
print('La inversa de la matrix es: ', matriz1.gauss_jordan())

#matrix_test = Myarray1(3,3,[1,2,7,4,1,7,7,8,1], True)
#matrix_test2 = Myarray1(3,3,[1,0,0,-4,1,0,-7,0,1], True)
#print(matrix_test2@matrix_test)

#%%

# =============================================================================
# class Linear_system:
#     def __init__(self, coeficientes, coeficientes_size, independent):
#         if coeficientes_size * coeficientes_size == len(coeficientes):
#             self.coeficientes = Myarray1(coeficientes_size, coeficientes_size, coeficientes, True)
#             self.independent = Myarray1(coeficientes_size, 1, independent, True)
#         else:
#             print('No se puede resolver el sistema de ecuaciones si las ecuaciones tienen una cantidad de variables distinta')
#     def search_inverse(self):
#         inverse = self.coeficientes.inverse()
#         return inverse
#     def solution(self):
#         return self.search_inverse() @ self.independent
# =============================================================================

#sistemaecuaciones = Linear_system([3,1,4,10], 2, [2,8])
#print(sistemaecuaciones.solution())
...
class Myarray2:
    ...
    def scale_row(self, j, x, matrix_input = None):
        """ La funcion scale_row toma por argumentos dos numeros, j y x, j representando una fila de la
        matriz y x un factor. Devuelve la misma matriz pero con la fila j multiplicada por x. La funcion 
        puede usarse para matrices externas a la instancia introduciendolas en el tercer parametro. X debe
        ser un entero y j debe estar dentro de la matriz """
