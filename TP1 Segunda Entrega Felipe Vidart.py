# -*- coding: utf-8 -*-
"""
Created on Fri Mar 31 19:52:55 2023
...
class Myarray1:
    ...
    def del_col_eye(self, j):
        matrix = self.to_by_row()
        aux_instance = Myarray1(self.r, self.c, matrix, True)
        identity = self.eye(self.c)
        updated_identity = identity.del_col(j)
    ...
    def swap_rows_eye(self, j, k):
        ...
        return self.return_byrow(swapped).elems
    ...
    def inverse(self):
        #para poder calcular la inversa de una matriz debe ser cuadrada y el determinante debe ser distinto de 0
        if self.r == self.c and self.det() != 0:
            matrix = self.to_by_row()
            cofactors_matrix = []
            for i in range(1, len(matrix)+1):
                #hago una instancia auxiliar
                aux_instance = Myarray1(self.r, self.c, matrix, True)
                #busco las cordenadas de cada elemento y le elimino esa fila y esa columna
                coords = aux_instance.get_coords(i)
                deletedrow = aux_instance.del_row(coords[0])
                deletedcolandrow = deletedrow.del_col(coords[1]) 
                #calculo el determinante de la submatriz que quedo sin esa columna y esa fila, multiplico por (-1) ** row*column para que queden bien los signos
                determinant = deletedcolandrow.det() * ((-1)**(coords[1]+coords[0]))
                #appendeo cada determinante a la matriz de cofactores
                cofactors_matrix.append(determinant)
            #creo la instancia a la que le voy a calcular el determinante
            main_instance = Myarray1(self.r, self.c, matrix, True)
            #creo una instancia cuyos elementos son la matriz de cofactores y hago la traspuesta
            cofactors_instance = Myarray1(self.r, self.c, cofactors_matrix, True)
            cofactors_instance.transpose()
            matrix_det = main_instance.det()
            #cbusco la inversa y la paso a by_row o by_column según se necesite
...
class Myarray2:
    ...
    def swap_rows_eye(self, j, k):
        ...
        for row in identity.elems:
            ...
            counter += 1
        swapped = identity @ aux_instance
        return self.return_byrow(self.flatten(swapped)).elems
    ...
    def det(self, matrix_input = None, rows = None, columns = None):
        ...
        if rows == 2 and columns == 2:
            ...
            for i in range(0,columns):
                element = first_row[i]
                #agarro el primer elemento de la primera fila para multiplicar por el determinante de la submatriz
                new_matrix = []
                for a in range(2, rows+1):
                    #para todas las filas menos la primera busco la fila
                    row = matrix.get_row(a).copy()
                    if len(row) > 1:
                        #si la row mide mas de 1 le saco el elemento que no entra en el determinante de 2x2 (saco la columna 1 de todas las filas, por ejemplo)
                        row.pop(col_counter)
                        new_matrix.extend(row)
                        #agrego la lista a a new matrix, me queda la matriz de 2x2 (en el caso de que lo runee con una 3x3)
                    elif len(row) <= 1:
                        break
                col_counter += 1
                #parte recursiva, se multiplica el elemento que agarre cuando arranco el for por el determinante de la submatriz que esta abajo a la derecha
                det += element * ((-1)**(1+col_counter)) * self.det(new_matrix, rows-1, columns-1)
        return det
    ...
    def inverse(self):
        ...
        if self.r == self.c and self.det() != 0:
            ...
            for i in range(1, len(self.flatten(matrix))+1):
                #hago una instancia auxiliar
                aux_instance = Myarray1(self.r, self.c, self.flatten(matrix), True)
                #busco las cordenadas de cada elemento y le elimino esa fila y esa columna
                coords = aux_instance.get_coords(i)
                deletedrow = aux_instance.del_row(coords[0])
                deletedcolandrow = deletedrow.del_col(coords[1])
                ...
                cofactors_matrix.append(determinant)
            #creo la instancia a la que le voy a calcular el determinante
            main_instance = Myarray1(self.r, self.c, self.flatten(matrix), True)
            #creo una instancia cuyos elementos son la matriz de cofactores y hago la traspuesta
            cofactors_instance = Myarray1(self.r, self.c, cofactors_matrix, True)
            cofactors_instance.transpose()
            matrix_det = main_instance.det()
    ...
    def __add__(self, other):
        if isinstance(other, int) or isinstance(other, float):
            ...
            if len(other.elems) != len(self.elems):
                ...
                #si estan por columna los paso a por fila para que se puedan sumar mas facilmente
                matrix1, matrix2 = self.to_by_row(), other.to_by_row()
    ...
    def __rmul__(self,other):
        if isinstance(other, int) or isinstance(other, float):
            return self * other
        #en este caso la multiplicacion es igual de izquierda a derecha que de derecha a izquieda porque es elemento a elemento
...
if __name__ == "__main__":
    ...
    #A PARTIR DE ACA ES LO QUE FUI PROBANDO PARA LA SEGUNDA PARTE DEL TP

    matrizdeprueba1 = Myarray1(3, 2, [1,2,3,4,5,6], True)
    matrizdeprueba2 = Myarray1(3, 2, [1,2,3,4,5,6], True)
    ...
    #matrizdeprueba1.eye(3).__dict__
...
if __name__ == "__main__":
    ...
    #A PARTIR DE ACA ES LO QUE PRUEBO PARA LA SEGUNDA PARTE DEL TP
    
    matrixprueba = Myarray2(3, 3, [1,2,3,4,5,6,7,8,9], True)
    matrixprueba2 = Myarray2(3, 3, [1,2,3,4,5,6,7,8,9], True)

    #sumamatrix = matrixprueba + matrixprueba2
    #sumanumero = matrixprueba + 10
    #rsumanumero = 10 + matrixprueba
    #print(sumamatrix, sumanumero, rsumanumero)

    #submatrix = matrixprueba - matrixprueba2
    #subnumero = matrixprueba - 10
    #rsubnumero = 10 - matrixprueba
    #print(submatrix, subnumero, rsubnumero)

    #mulmatrix = matrixprueba * matrixprueba2
    #mulnumero = matrixprueba * 10
    #rmulnumero = 10 * matrixprueba
    #print(mulmatrix, mulnumero, rmulnumero)

    #matmulprueba = matrixprueba @ matrixprueba2
    #print(matmulprueba)

    #powprueba = matrixprueba ** 3
    #print(powprueba)

    #pruebadelrowidentidad = matrixprueba.del_row_eye(1)
    #print(pruebadelrowidentidad)
    #pruebadelcolidentidad = matrixprueba.del_col_eye(1)
    #print(pruebadelcolidentidad)

    #pruebaswaprowseye = matrixprueba.swap_rows_eye(2, 3)
    #print(pruebaswaprowseye)
    #pruebaswapcolseye = matrixprueba.swap_cols_eye(1, 2)
    #print(pruebaswapcolseye)

    #matrixpruebadet = Myarray2(4,4,[3,3,4,7,3,6,8,4,3,1,4,6,8,4,3,1], True)
    #print(matrixpruebadet.det())
